...
func ParseInetDiagMsg(data []byte) IDM {
	// TODO - also decode the RTA, using RtAttr golang.org/x/sys/unix

	var idm InetDiagMsg
	return IDM{*(*InetDiagMsg)(unsafe.Pointer(&data[0])), data[rtaAlignOf(int(unsafe.Sizeof(idm))):]}
}
