...
func TestParseInetDiagMsg(t *testing.T) {
	...
	raw, value := parse.SplitInetDiagMsg(data[:])
	hdr, err := raw.Parse()
	rtx.Must(err, "")
	...
	raw, value = parse.SplitInetDiagMsg(data[:1])
...
func TestID4(t *testing.T) {
	...
	*(*uint16)(unsafe.Pointer(&data[srcPortOffset])) = 0x1234
	...
	raw, _ := parse.SplitInetDiagMsg(data[:])
	hdr, err := raw.Parse()
	rtx.Must(err, "")
...
func TestParseGarbage(t *testing.T) {
	...
	_, err = parse.ParseNetlinkMessage(&badNm, true)
	if err == nil {
	...
	nm.Header.Type = 10
	_, err = parse.ParseNetlinkMessage(&nm, false)
	if err == nil {
...
func TestCompare(t *testing.T) {
	...
	mp1, err := parse.ParseNetlinkMessage(&nm, true)
	if err != nil {
		log.Fatal(err)
	}
	...
	mp2, err := parse.ParseNetlinkMessage(&nm2, true)
	if err != nil {
		log.Fatal(err)
	}
	...
	diff, err = mp1.Compare(mp2)
	rtx.Must(err, "")
	if diff != parse.StateOrCounterChange {
...
func TestNLMsgSerialize(t *testing.T) {
	...
	for {
		...
		pm, err := parse.ParseNetlinkMessage(msg, false)
...
func TestCompressionSize(t *testing.T) {
	...
	log.Println("Reading messages from", source)
	rdr := zstd.NewReader(source)
	msgs := make([]*parse.ParsedMessage, 0, 200)
...
func BenchmarkNLMsgParseSerializeCompress(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, msg := range raw {
			m, err := parse.ParseNetlinkMessage(msg, false)
			rtx.Must(err, "Could not parse test data")
			jsonBytes, err := json.Marshal(m)
			rtx.Must(err, "Could not serialize %v", m)
			w.Write(jsonBytes)
